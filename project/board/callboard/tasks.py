...
def respond_send_email(respond_id):
    ...
    send_mail(
        subject=f'MMORPG Callboard: новый отклик на объявление!',
...
def respond_accept_send_email(response_id):
    ...
    send_mail(
        subject=f'MMORPG Callboard: Ваш отклик принят!',
...
def send_mail_monday_8am():
    now = timezone.now()
    list_week_posts = list(Post.objects.filter(dateCreatin__gte=now - timedelta(days=7)))
    if list_week_posts:
